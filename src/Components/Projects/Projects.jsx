...
const Projects = () => {
  ...
  const sortedProjects = projects?.sort(
    (project1, project2) => project1?.projectId - project2?.projectId
  );
            ...
            sortedProjects?.map((project, index) => {
              return (
                <SwiperSlide
                  ...
                  key={index}
                  className={` pb-5 flex    gap-10 border-b shadow-lg shadow-stone-500 flex-col   p-3 rounded-lg bg-gray-50  h-full  justify-start mb-2`}
                >
                  {/* image div  */}
                  <div className="">
                    <Image
                  ...
                  <div className=" flex-1 text-start justify-start px-10">
                    ...
                    <div className=" my-5">
                      ...
                      <ul>
                        {project.features?.map((feature) => (
                          <li
                            className=" text-sm lg:text-base list-disc ml-8 "
                            key={feature}
                          >
                            {feature}
                          </li>
                    ...
                    <div className=" grid grid-cols-2 md:grid-cols-5 lg:grid-cols-7  gap-y-2 lg:gap-y-5  gap-x-3 ">
                      {project?.techs?.map((tech) => {
                        return (
                          <div key={tech}>
                            <h3 className=" bg-[#575550] text-white text-sm p-1 text-center  rounded-md w-full ">
                              {tech}
                            </h3>
                          </div>
                        );
                    ...
                    <div>
                      ...
                      <div className=" grid grid-cols-2  text-center  md:grid-cols-3 justify-center gap-3 w-full lg:w-[60%]  lg:gap-4 ">
                        <Link
                          ...
                          target="_blank"
                        >
                          <button className="  flex gap-2 md:gap-5 items-center Btn-Primary">
                            Live Site{" "}
                            <FaArrowUpRightFromSquare></FaArrowUpRightFromSquare>
                          </button>
                        </Link>
                        <Link
                          href={project?.clientRepo ? project?.clientRepo : ""}
                          target="_blank"
                        >
                          <button className=" flex gap-5 items-center Btn-Secondary">
                            <FaGithub className=" text-base lg:text-2xl"></FaGithub> Client
                          </button>
                        </Link>
                        {project?.serverRepo?       <Link
                          href={
                            project?.serverRepo
                          }
                          target="_blank"
                        >
                          <button className=" flex gap-5 items-center Btn-Secondary">
                            <FaGithub className=" text-base lg:text-2xl"></FaGithub> Server
                          </button>
                        </Link>:""}
