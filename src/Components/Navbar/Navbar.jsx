  ...
  return (
    <nav className='  bg-gray-100 bg-opacity-60 h-16 flex items-center'>
      ...
      <div  className=' w-[95%] lg:w-[85%] mx-auto flex justify-between'>
        {/* logo  */}
     <div>
     <h1 className=' hover:scale-105 hover:bg-green-100 cursor-pointer w-36 text-2xl font-bold gap-x-0 relative '><span className='  '>H</span> H {" "}
       <span className='pr-3 '>{"Redo"}</span>
        <span className=' top-2 right-5 text-4xl absolute'>{"`"}</span>
        <span className='  absolute text-green-600 top-1.5 rotate-6 right-4'>{"/"}</span>
        </h1>
     </div>
        ...
        <div>
          ...
          <div className=' block md:hidden'>
            <button onClick={()=>setShowLinks(!showLinks)} ><GiHamburgerMenu className=' text-2xl'></GiHamburgerMenu></button>
           {
            ...
            {navLinks?.map((links)=>(
              <Link key={links.to} className={`font-bold text-lg hover:scale-105`} href={links.to||''}
        ...
        <div className=' hidden md:flex'>
          <ul className=' flex gap-5  '>
            {/* render all links by map  */}
            {navLinks?.map((links)=>(
              <Link key={links.to||''} className={`font-bold text-lg hover:scale-105 hover:text-green-700`} href={links.to}
